...
extension TabBarCoordinator: TabBarCoordinatorProtocol {
  func getGeneralRouter() -> GeneralRoutableProtocol? {
    childCoordinators.filter({ $0.type == .general }).first as? GeneralRoutableProtocol
  }
  
  func getFavoriteRouter() -> FavoriteRoutableProtocol? {
    childCoordinators.filter({ $0.type == .favorites }).first as? FavoriteRoutableProtocol
  }
  
  func getProfileRouter() -> ProfileRoutableProtocol? {
    childCoordinators.filter({ $0.type == .profile }).first as? ProfileRoutableProtocol
  }
